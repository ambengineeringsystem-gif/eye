...
// audio element to play when alarm is armed remotely
const armedAudio = document.getElementById('armedAudio');

// Attempt to unlock audio on first user interaction or during startListening
let __audioUnlocked = false;
async function tryUnlockAudioOnce(){
  if(__audioUnlocked) return;
  try{
    // Resume/create AudioContext (helps unlocking on some browsers)
    const AC = window.AudioContext || window.webkitAudioContext;
    if(AC){
      try{ const ac = new AC(); if(ac && ac.state === 'suspended') await ac.resume(); window.__audioContext = ac; }catch(e){ /* ignore */ }
    }
    // Play a very short muted playback on the armedAudio element to unlock playback
    if(armedAudio){
      try{ armedAudio.muted = true; await armedAudio.play().catch(()=>{}); armedAudio.pause(); armedAudio.currentTime = 0; armedAudio.muted = false; }
      catch(e){ /* ignore */ }
    }
    __audioUnlocked = true;
    console.debug('Audio unlocked');
  }catch(e){ console.warn('tryUnlockAudioOnce failed', e); }
}
...
enableAudioBtn && (enableAudioBtn.onclick = async ()=>{ try{ await tryUnlockAudioOnce(); status('Audio enabled'); }catch(e){ console.warn('enableAudio failed', e); } });
...
async function startListening(){
  ...
  try{ await tryUnlockAudioOnce(); }catch(e){}
...
async function registerCamera(){
  ...
  try{
    const statusListener = onValue(camRef, snap => {
      const data = snap.val();
      if(!data) return;
      // if an alarmArmed timestamp is written, play the armed sound once
      try{
        if(data.alarmArmed){
          // Support both legacy numeric timestamp and new structured object {ts, by, armed}
          let armedTs = 0;
          let armedFlag = true;
          try{
            if(typeof data.alarmArmed === 'object'){
              armedTs = Number(data.alarmArmed.ts) || 0;
              if(typeof data.alarmArmed.armed !== 'undefined') armedFlag = !!data.alarmArmed.armed;
            } else {
              armedTs = Number(data.alarmArmed) || 0;
              armedFlag = true;
            }
          }catch(e){ armedTs = Number(data.alarmArmed) || 0; armedFlag = true; }

          const lastSeen = localStorage.getItem('lastAlarmArmedTs:' + name) || '0';
          const lastNum = Number(lastSeen) || 0;
          if(armedTs > lastNum){
            localStorage.setItem('lastAlarmArmedTs:' + name, String(armedTs));
            try{
              if(armedFlag){
                if(armedAudio){
                  armedAudio.pause(); armedAudio.currentTime = 0; armedAudio.volume = 0.95;
                  armedAudio.play().catch(e=>{
                    console.warn('armed audio play failed', e);
                    try{ status('Audio blocked. Click anywhere to enable alarm sounds.'); }catch(e){}
                    const listener = async ()=>{
                      try{ await tryUnlockAudioOnce(); if(armedAudio){ armedAudio.play().catch(err=>console.warn('play after unlock failed', err)); } }
                      catch(err){ console.warn('unlock+play failed', err); }
                ...
                // disarm event — update status text
                try{ status('Alarm disarmed remotely'); }catch(e){}
              }
            }catch(e){ console.warn('play armed audio failed', e); }
          }
        }
      }catch(e){ console.warn('alarmArmed handling failed', e); }
